...
Sıralı konteynırlar elemanlarının sırasını tutan konteynır sınıflardır. Sıralı konteynırların tanımlayıcı özelliği elemanları istenilen konuma göre ekleyebilmektir. Sıralı konteynırlar için en yaygın örnek dizi(array)'dir: eğer diziye 4 adet eleman eklenirse, elemanlar eklenme sırasıyla aynı sıralanacaktır.

STL kütüphanesi 6 sıralı konteynır içerir: std::vector, std::deque, std::array, std::list, std::forward_list, and std::basic_string.

==== Vector

Vektörler bir eleman eklendiğinde veya silindiğinde boyutunu otomatik olarak ayarlayan dinamik dizilerdir. Vektör elemanları sıralı olarak konumlanırlar böylece iteratör kullanarak erişilebilir ve üzerinde dolaşılabilir. Vektörlerde veri sona eklenir. 
...
int main()
{
    ...
    for(vector::iterator itr = vec.begin(); itr != vec.end(); ++itr)//auto kullanmadan iteratör tanımlaması
        cout << *itr << " ";

    cout << endl;

    for(vector::reverse_iterator itr = vec.rbegin(); itr != vec.rend(); ++itr)//auto kullanmadan reverse iteratör tanımlaması
        cout << *itr << " ";//reverse iteratör vektörün sonundan başlayıp ilk elemana doğru ilerler
    // rbegin() ve rend() sadece reverse iteratörle kullanılır : rbegin() son elemanı, rend() ise ilk elemanı gösterir

    cout << endl;

    for(auto itr = vec.begin(); itr != vec.end(); ++itr)//auto ile iteratör tanımlaması
        cout << *itr << " ";

    cout << endl;

    for(auto itr = vec.rbegin(); itr != vec.rend(); ++itr)//auto ile reverse iteratör tanımlaması
        cout << *itr << " ";//görüldüğü gibi rbegin() ve rend() kullanımından auto deyimiyle derleyici
                            // bunun reverse iteratör olduğunu anladı
...
int main()
{
    ...
    cout << "Boyut: " << vec.size() << endl;//vec.size() vektördeki eleman sayısını döndürür
    cout << "Kapasite: " << vec.capacity() << endl;//vec.capacity() vektör için ayrılan yer miktarını gösterir
    cout << "Max boyut: " << vec.max_size() << endl;//vec.max_size() vektörün alabileceği maksimum eleman sayısını döndürür
